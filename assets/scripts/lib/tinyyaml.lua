...
-- Vendored from https://github.com/peposso/lua-tinyyaml (MIT License,
-- Copyright (c) 2017 peposso). Patched to fix a multi-return-value bug in
-- `parseseq`/`parsedocuments` that corrupts `table.insert` calls when a
-- sequence item (or document root) is a flow-style `{...}`/`[...]` value
-- (see the two `-- Parenthesized to truncate...` comments below).
-------------------------------------------------------------------------------
...
local function parseseq(line, lines, indent)
  ...
  while #lines > 0 do
    ...
    elseif rest then
      ...
      tremove(lines, 1)
      -- Parenthesized to truncate to one value: `parsescalar` tail-calls
      -- `parseflowstyle` for `{...}`/`[...]` values, which returns
      -- `(value, leftover_line)`; without this, `tinsert` sees 3 args and
      -- misreads the parsed table as its numeric `pos` argument.
  ...
  while #lines > 0 do
    ...
    if docright then
      if (not sfind(docright, '^%s+$') and
        ...
        tinsert(root, (parsescalar(docright, lines)))  -- see parseseq's identical fix above
      end
